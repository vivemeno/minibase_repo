...
public class Phase3 {
    ...
    public void IndexJoinWithTagIndex( Map<String, String> tagMapping, List<Rule> rules) {
        ...
        try {
            prevIterator = new NestedLoopsJoins(baseTableAttrTypes, 2, baseTableStringLengths, baseTableAttrTypes, 2,
                    baseTableStringLengths, 10, fileScanner, "nodesSortedOnTag.in", filterConditions, innerRelFilterConditions, currProjection, 4, "Composite.in", 0);
        } catch (Exception e) {
        ...
        for (int x = 1; x < rules.size(); ++x) {
            ...
            innerRelFilterConditions[0] = new CondExpr();
            innerRelFilterConditions[1] = new CondExpr();
            setConditions(filterConditions, innerRelFilterConditions, currRule, tagOffsetMap.get(currRule.outerTag), false, tagMapping);
            ...
            try {
                currIterator = new NestedLoopsJoins(joinedTableAttrTypes, 2 * ruleNumber, joinedTableStringLengths, baseTableAttrTypes, 2,
                        baseTableStringLengths, 10, prevIterator, "nodesSortedOnTag.in", filterConditions, innerRelFilterConditions, currProjection,
    ...
    private void setConditions(CondExpr[] outFilter, CondExpr[] rightFilter, Rule rule, int offset, boolean isFirstRule,  Map<String, String> tagMapping) {
