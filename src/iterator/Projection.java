...
public class Projection {
	 ...
	 * @param          perm_mat[] shows what input fields go where in the output
	 *                 tuple
	...
	public static void Join(int physop, Tuple t1, AttrType type1[], Tuple t2, AttrType type2[], Tuple Jtuple, FldSpec perm_mat[],
			int nOutFlds) throws UnknowAttrType, FieldNumberOutOfBoundException, IOException {
		...
		for (int i = 0; i < nOutFlds; i++) {
			...
			case RelSpec.outer: // Field of outer (t1)
				switch (type1[perm_mat[i].offset - 1].attrType) {
				case AttrType.attrInteger:
					Jtuple.setIntFld(i + 2, t1.getIntFld(perm_mat[i].offset));
					break;
				case AttrType.attrReal:
					Jtuple.setFloFld(i + 2, t1.getFloFld(perm_mat[i].offset));
					break;
				case AttrType.attrString:
					Jtuple.setStrFld(i + 2, t1.getStrFld(perm_mat[i].offset));
					break;
				case AttrType.attrInterval:
					Jtuple.setIntervalFld(i + 2, t1.getIntervalField(perm_mat[i].offset));
					break;
				default:
			...
			case RelSpec.innerRel: // Field of inner (t2)
				switch (type2[perm_mat[i].offset - 1].attrType) {
				case AttrType.attrInteger:
					Jtuple.setIntFld(i + 2, t2.getIntFld(perm_mat[i].offset));
					break;
				case AttrType.attrReal:
					Jtuple.setFloFld(i + 2, t2.getFloFld(perm_mat[i].offset));
					break;
				case AttrType.attrString:
					Jtuple.setStrFld(i + 2, t2.getStrFld(perm_mat[i].offset));
					break;
				case AttrType.attrInterval:
					Jtuple.setIntervalFld(i + 2, t2.getIntervalField(perm_mat[i].offset));
					break;
				default:
